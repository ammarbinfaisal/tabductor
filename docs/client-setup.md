...
3. Call `browser_actionables` first when you need DOM refs. It returns a lightweight actionable ref inventory.
4. Call `browser_describe_ref` when one specific ref needs deeper context.
5. Call `browser_snapshot` only when you need broader page context than the actionable list provides.
6. Use action tools normally and let the MCP return change summaries from its cache.
7. Call `browser_state` or `browser_snapshot` only when you need more detail than the action response already provides.
...
- compact discovery snapshots plus explicit per-ref elaboration
